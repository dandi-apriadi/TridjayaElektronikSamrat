...
pub async fn send_template(
    ...
    let vars: Vec<String> =
        serde_json::from_str(&template.1.unwrap_or_else(|| "[]".to_string())).unwrap_or_default();
...
pub async fn send_media(
        ...
        match field.name().unwrap_or("") {
            "session_id" => {
                session_id = Some(field.text().await.map_err(|e| {
                    tracing::error!("Field error: {}", e);
                    AppError::Internal
                })?)
            }
            "to" => {
                to = Some(field.text().await.map_err(|e| {
                    tracing::error!("Field error: {}", e);
                    AppError::Internal
                })?)
            }
            ...
            "type" => {
                media_type = Some(field.text().await.map_err(|e| {
                    tracing::error!("Field error: {}", e);
                    AppError::Internal
                })?)
            }
    ...
    let media_path = if let Some(data) = file_data {
        ...
        tokio::fs::create_dir_all(format!("{}/wa_media", uploads_dir))
            .await
            .map_err(|e| {
...
async fn send_via_bridge(
    ...
    let result = bridge
        .send_request(session_id, "send_message".to_string(), params)
        .await?;
