...
impl BridgeProcess {
    ...
    fn new(
        ...
        let expected_shutdown_clone = expected_shutdown.clone();
                ...
                Ok(None) => {
                    ...
                    if !expected_shutdown.load(Ordering::SeqCst) {
                        let event = BridgeEvent {
                            ...
                            data: serde_json::json!({ "error": "bridge process exited unexpectedly" }),
                        };
...
impl BridgeClient {
    ...
    pub async fn spawn_process(&self, session_id: String) -> BridgeResult<()> {
        ...
        let std_stdout = child
            .stdout
            .take()
            .ok_or_else(|| BridgeError::SpawnFailed("Failed to capture stdout".to_string()))?;
        let stdout = tokio::process::ChildStdout::from_std(std_stdout).map_err(|e| {
            BridgeError::SpawnFailed(format!("Failed to convert stdout to async: {}", e))
        })?;
    ...
    pub async fn send_request(
        ...
        self.send_request_with_timeout(session_id, method, params, DEFAULT_TIMEOUT)
            .await
    }
