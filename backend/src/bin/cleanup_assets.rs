...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let rows: Vec<(Option<String>, Option<String>)> =
        sqlx::query_as("SELECT image, images FROM products")
            .fetch_all(&pool)
            .await?;
    for (img, imgs) in rows {
        if let Some(path) = img {
            used_assets.insert(normalize_path(&path));
        }
    ...
    let rows: Vec<(Option<String>, Option<String>)> =
        sqlx::query_as("SELECT author_image, hero_image FROM blog_posts")
            .fetch_all(&pool)
            .await?;
    for (a, h) in rows {
        if let Some(path) = a {
    ...
    let rows: Vec<(Option<String>,)> = sqlx::query_as("SELECT image FROM promos")
        .fetch_all(&pool)
        .await?;
    for (img,) in rows {
        if let Some(path) = img {
    ...
    let rows: Vec<(Option<String>,)> = sqlx::query_as("SELECT avatar FROM users")
        .fetch_all(&pool)
        .await?;
    for (img,) in rows {
        if let Some(path) = img {
    ...
    let rows: Vec<(Option<String>,)> = sqlx::query_as("SELECT logo_url FROM partners")
        .fetch_all(&pool)
        .await?;
    for (img,) in rows {
        if let Some(path) = img {
    ...
    let rows: Vec<(Option<String>, Option<String>)> =
        sqlx::query_as("SELECT profile_photo, ktp_photo FROM agent_registrations")
            .fetch_all(&pool)
            .await?;
    for (p, k) in rows {
        if let Some(path) = p {
    ...
    println!(
        "Total aset unik yang direferensikan di database: {}",
        used_assets.len()
    );
    ...
    if !uploads_dir.exists() {
        ...
        println!(
            "Ditemukan {} file aset yang tidak digunakan.",
            orphans.len()
        );
